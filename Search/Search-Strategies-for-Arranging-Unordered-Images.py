...
class ImageAgent(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.top = np.random.randn(1,10)
        self.bottom = np.random.randn(1,10) 
        self.left = np.random.randn(1,10)
        self.right = np.random.randn(1,10)
        
        #estimates of values beyond the image border
        #generate 100 random pixel values for each side
        self.topForecast = np.random.randn(1,10)
        self.bottomForecast = np.random.randn(1,10) 
        self.leftForecast = np.random.randn(1,10)
        self.rightForecast = np.random.randn(1,10)
...
continueApplication = input("\n\nPRESS Y TO ARRANGE THE IMAGES USING PLAN & SEARCH\n\n")
...
def findNextImage2(image, side, searchNumber,searchRank,listOfImagesUsed):
    ...
    print("Image "+str(int(image))+" has image "+str(int(nextImage))+" as its best "+str(side)+" target image")
...
continueApplication = input("\n\nPRESS Y TO ARRANGE THE IMAGES USING GREEDY SEARCH\n\n")
...
print(sequence1)
print("\nThe Plan and Search sequence was completed in " + str(round((t4-t3)+(t2-t1),2)) + " seconds")
